...
class BidirectionalFoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        foods = self.foodGrid.asList() #make all food a list
        self.start = (self.init_pos, foods) 
    ...
    def getGoalStates(self):
        goal_states = [] #to store food/goal states
        """YOUR CODE HERE FOR TASK 3:"""
        # You must generate all goal states
        foods = self.foodGrid.asList() #make all food a list
        for food in foods: 
            goal_states.append((food, [])) #append each food into the goal state list one by one
        return goal_states

    def isGoalState(self, state):
        goal_achieved = state[1].count() == 0 #no element in goal state means goal achieved
        """YOUR CODE HERE FOR TASK 3:"""
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_x][next_y]:
                nextState = (next_x, next_y)
                foodGrid = Grid(self.foodGrid.width, self.foodGrid.height)
                for (x0, y0) in state[1]:
                    foodGrid[x0][y0] = True
                foodGrid[next_x][next_y] = False
                nextFood = foodGrid.asList()
    ...
    def getBackwardsSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_x][next_y]:
                nextState = (next_x, next_y)
                rev_action = Actions.reverseDirection(action)
                emptyFoodGrid = Grid(self.foodGrid.width, self.foodGrid.height)
                for (x0,y0) in state[1]:
                    emptyFoodGrid[x0][y0] = True
                emptyFoodGrid[x][y] = self.foodGrid[x][y]
                nextFood = emptyFoodGrid.asList()
                successors.append(((nextState,nextFood), rev_action, 1))
        return successors
